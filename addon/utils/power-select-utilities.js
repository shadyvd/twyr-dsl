...
export function countOptions(collection) {
	...
	(function walk(collection) {
		...
		for (let idx = 0; idx < get(collection, 'length'); idx++) {
			const entry = collection.objectAt ? collection.objectAt(idx) : collection[idx];
			if(isGroup(entry))
...
export function defaultHighlighted({
	...
	const option = highlightedOption || selected;
...
export function filterOptions(options, text, matcher, skipDisabled = false) {
	...
	for (let idx = 0; idx < length; idx++) {
		const entry = options.objectAt ? options.objectAt(idx) : options[idx];
		if(!skipDisabled || !get(entry, 'disabled')) {
			if(isGroup(entry)) {
				const suboptions = filterOptions(get(entry, 'options'), text, matcher, skipDisabled);
...
export function indexOfOption(collection, option) {
	...
	return function walk(collection) {
		...
		for(let idx = 0; idx < get(collection, 'length'); idx++) {
			...
			if(isGroup(entry)) {
				const result = walk(get(entry, 'options'));
...
export function optionAtIndex(collection, index) {
	let counter = 0;
	...
	return function walk(options, ancestorIsDisabled) {
		...
		while (counter <= index && localCounter < length) {
			...
			if(isGroup(entry)) {
				...
				return {
					'disabled': ancestorIsDisabled || !!get(entry, 'disabled'),
					'option': entry
				};
			}
			else {
